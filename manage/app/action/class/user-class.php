...
{
    $stmt = $this->con->prepare("SELECT * FROM feedback ORDER BY id DESC");
    $stmt->execute();  
    return $stmt;
}
...
public function get_district_feedback_list($district)
{
    $stmt = $this->con->prepare("SELECT * FROM feedback WHERE district='$district' ORDER BY id DESC");
    $stmt->execute();  
    return $stmt;
}
